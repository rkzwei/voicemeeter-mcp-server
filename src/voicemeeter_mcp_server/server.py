        ...
        async def handle_read_resource(uri: str) -> str:
            ...
            if uri == "voicemeeter://status":
                return json.dumps({
                    ...
                    "type": self.vm_api.voicemeeter_type.name if self.vm_api.voicemeeter_type else None,
                ...
                params = [
                    "mute", "mono", "solo", "gain", "comp", "gate", "limit",
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            try:
                if name == "voicemeeter_connect":
                    ...
                    return [TextContent(
                        type="text",
                        text="Disconnected from Voicemeeter" if success else "Failed to disconnect"
                    )]
                    ...
                    if not self.vm_api.is_connected:
                        if not self.vm_api.login():
                            return [TextContent(
                                type="text",
                                text="Failed to connect to Voicemeeter"
                            )]
                    ...
                    if value is not None:
                        return [TextContent(
                            type="text",
                            text=f"Parameter '{parameter}' = {value}"
                        )]
                    else:
                        return [TextContent(
                            type="text",
                    ...
                    if param_type == "string":
                        success = self.vm_api.set_parameter_string(parameter, str(value))
                    else:
                        success = self.vm_api.set_parameter_float(parameter, float(value))
                        ...
                        for param in root.findall(".//param"):
                            ...
                            if name and value is not None:
                                try:
                                    ...
                                    if self.vm_api.set_parameter_float(name, float_value):
                                        applied_count += 1
                                except ValueError:
                                    # Try as string
