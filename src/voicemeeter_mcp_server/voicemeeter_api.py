...
class VoicemeeterAPI:
        ...
        try:
            ...
            if is_64bit:
                ...
                dll_paths.extend([
                    ...
                    os.path.join(os.environ.get("PROGRAMFILES(X86)", ""), "VB", "Voicemeeter", "VoicemeeterRemote64.dll"),
                    os.path.join(os.environ.get("WINDIR", ""), "System32", "VoicemeeterRemote64.dll"),
                    ...
                    os.path.join(os.environ.get("WINDIR", ""), "SysWOW64", "VoicemeeterRemote.dll"),
                ])
            else:
                # 32-bit system - use 32-bit DLL only
                dll_paths.extend([
                    "VoicemeeterRemote.dll",  # Try from PATH first
                    os.path.join(os.environ.get("PROGRAMFILES", ""), "VB", "Voicemeeter", "VoicemeeterRemote.dll"),
                    os.path.join(os.environ.get("WINDIR", ""), "System32", "VoicemeeterRemote.dll"),
                ])
    ...
    def get_level(self, level_type: int, channel: int) -> Optional[float]:
        ...
        try:
            ...
            get_level_func.argtypes = [ctypes.c_long, ctypes.c_long, ctypes.POINTER(ctypes.c_float)]
