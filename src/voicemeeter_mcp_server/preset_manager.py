...
class PresetManager:
    ...
    def __init__(self, preset_dir: str = "presets", backup_dir: str = "presets/backups"):
        ...
        logger.info(f"PresetManager initialized with preset_dir={preset_dir}, backup_dir={backup_dir}")
    ...
    def load_xml_preset(self, xml_path: str) -> VoicemeeterPreset:
        ...
        try:
            ...
            if tags_elem is not None:
                metadata.tags = [tag.text for tag in tags_elem.findall('tag') if tag.text]
                ...
                for strip_elem in strips_elem.findall('strip'):
                    ...
                    for param_elem in strip_elem.findall('param'):
                        ...
                        if param_name and param_value is not None:
                            ...
                            parameters.append(PresetParameter(
                                name=param_name,
                                value=param_value
                            ))
            ...
            if scenarios_elem is not None:
                for scenario_elem in scenarios_elem.findall('scenario'):
                    ...
                    scenarios.append(PresetScenario(
                        name=scenario_name,
    ...
    def load_preset_json(self, json_path: str) -> VoicemeeterPreset:
        ...
        strips = [
            PresetStrip(
                ...
                parameters=[PresetParameter(**param) for param in strip_data['parameters']]
            )
        ...
        buses = [
            PresetBus(
                ...
                parameters=[PresetParameter(**param) for param in bus_data['parameters']]
            )
        ...
        scenarios = [
            PresetScenario(
                ...
                parameters=[PresetParameter(**param) for param in scenario_data['parameters']]
            )
    ...
    def compare_presets(self, preset1: VoicemeeterPreset, preset2: VoicemeeterPreset) -> Dict[str, Any]:
        ...
        for strip_id in all_strip_ids:
            ...
            if strip1 is None:
                comparison["strip_changes"][strip_id] = {"status": "added", "parameters": strip2.parameters}
                comparison["summary"]["strips_modified"] += 1
            elif strip2 is None:
                comparison["strip_changes"][strip_id] = {"status": "removed", "parameters": strip1.parameters}
                comparison["summary"]["strips_modified"] += 1
            else:
                # Compare parameters
                ...
                if param_changes:
                    comparison["strip_changes"][strip_id] = {"status": "modified", "parameter_changes": param_changes}
                    comparison["summary"]["strips_modified"] += 1
        ...
        for bus_id in all_bus_ids:
            ...
            if bus1 is None:
                comparison["bus_changes"][bus_id] = {"status": "added", "parameters": bus2.parameters}
                comparison["summary"]["buses_modified"] += 1
            elif bus2 is None:
                comparison["bus_changes"][bus_id] = {"status": "removed", "parameters": bus1.parameters}
                comparison["summary"]["buses_modified"] += 1
            else:
                # Compare parameters
                ...
                if param_changes:
                    comparison["bus_changes"][bus_id] = {"status": "modified", "parameter_changes": param_changes}
                    comparison["summary"]["buses_modified"] += 1
        ...
        for scenario_name in all_scenario_names:
            ...
            if scenario1 is None:
                comparison["scenario_changes"][scenario_name] = {"status": "added", "parameters": scenario2.parameters}
                comparison["summary"]["scenarios_modified"] += 1
            elif scenario2 is None:
                comparison["scenario_changes"][scenario_name] = {"status": "removed", "parameters": scenario1.parameters}
                comparison["summary"]["scenarios_modified"] += 1
            else:
                # Compare parameters
                ...
                if param_changes:
                    comparison["scenario_changes"][scenario_name] = {"status": "modified", "parameter_changes": param_changes}
                    comparison["summary"]["scenarios_modified"] += 1
        ...
        logger.info(f"Preset comparison completed: {comparison['summary']['total_changes']} total changes")
        return comparison
            ...
            if preset_file.is_file():
                if extension is None or preset_file.suffix.lower() == extension.lower():
                    presets.append({
                        ...
                        "modified": datetime.fromtimestamp(preset_file.stat().st_mtime).isoformat()
                    })
    ...
    def create_template(self, template_name: str, voicemeeter_type: str = "potato") -> VoicemeeterPreset:
    ...
    def export_preset_xml(self, preset: VoicemeeterPreset, xml_path: str) -> None:
        ...
        if preset.metadata.voicemeeter_type:
            ET.SubElement(metadata_elem, "voicemeeter_type").text = preset.metadata.voicemeeter_type
        ...
        for scenario in preset.scenarios:
            scenario_elem = ET.SubElement(scenarios_elem, "scenario", name=scenario.name)
